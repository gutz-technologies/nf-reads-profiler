...
run_dir=benchmarks/graviton/execution/g5
mkdir -p "$run_dir"
date -u +%FT%TZ > "$run_dir/started.txt"
set +e
nextflow -log "$run_dir/nextflow.log" run main.nf -c benchmarks/graviton/g5.config -name graviton_bench_g5_20260918 -ansi-log false > "$run_dir/console.log" 2>&1
run_status=$?
set -e
...
archive_uri=s3://gutz-nf-reads-profilers-runs/results/graviton-benchmark-g5-20260918/execution/g5/
